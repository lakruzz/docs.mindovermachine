...
│   ├── astro.config.mjs    # Astro + Starlight configuration (sidebar, locales, redirects)
│   ├── package.json        # Astro app dependencies
│   ├── tsconfig.json
│   ├── public/             # Static assets (favicon: mom-favicon.ico)
│   └── src/
│       ├── env.d.ts
│       ├── content.config.ts       # Extended schema: redirectFrom, giscus fields
│       ├── assets/                 # Images: mom-logo-text-transparent.png, mom-coin.png
│       ├── styles/
│       │   └── custom.scss         # Custom SCSS overrides
│       ├── components/
│       │   ├── Giscus.astro        # Giscus comment widget
│       │   └── overrides/
│       │       └── Footer.astro    # Footer override: injects Giscus before default footer
│       ├── config/
│       │   └── frontmatter-redirects.mjs  # Reads redirect-from frontmatter → Astro redirects
│       └── content/
│           └── docs/               # All documentation pages (Markdown/MDX)
│               ├── da/             # PRIMARY locale (Danish)
│               │   ├── index.mdx
